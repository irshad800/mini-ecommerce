...
function App() {
  ...
  // 🐛 CORRECTION HERE: The URL must be wrapped in quotes to be a string.
  // The value is hardcoded for a quick fix.
  const API_URL = 'http://13.48.30.140:5000'; 

  console.log('API_URL:', API_URL); // Debug - should now log the URL string

  const fetchProducts = useCallback(async () => {
    setIsLoading(true);
    try {
      // API_URL is now a valid string: 'http://13.48.30.140:5000/products'
      ...
      // Note: If you see a CORS error now, the API is running, but access is blocked.
  ...
  const validateForm = () => {
    ...
    if (!form.stock || parseInt(form.stock) < 0) newErrors.stock = 'Stock must be a non-negative number';
    setErrors(newErrors);
                  ...
                  <div className="card border-0 shadow-sm">
                    <div className="card-body">
                      ...
                      <p className="card-text text-muted">
                        ${product.price ? product.price.toFixed(2) : 'N/A'} | {product.category} | Stock: {product.stock}
                      </p>
                    </div>
                  </div>
                </div>
